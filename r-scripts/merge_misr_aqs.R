...
for(i in 1:length(years)){
  ...
  for (d in as.list(dates)){
    ...
    year_aqs <- year_aqs %>%
      st_drop_geometry() %>%
      left_join(day_aqs, by=c("Date", "Site.Latitude", "Site.Longitude")) %>%
...
write.csv(aqs.merged,paste0(repo.dir, "data/merged/Merged_MISR_AQS_Matched_2000_2021.csv"), row.names = FALSE)
