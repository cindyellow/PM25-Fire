### For merging MISR-CSN
repo.dir = '/data/home/huan1766/PM25-Fire/'

## Capture messages and errors to a file.
zz <- file(paste0(repo.dir, "errors-logs/errors_merge_misr_csn.Rout"), open="wt")
...
remote.csn.dir = '/home/huan1766/remoteproject/PM25-Research/Data/MISR/MISR_merged_data/'
...
csn <- read.delim(paste0(remote.csn.dir, "MISR_CSN_Matched.csv"), sep=",", strip.white=TRUE, row.names=NULL)
...
in_bound <- lengths(st_intersects(csn, cal_bound))>0

# Get subset with indexing
in_cali_csn <- csn[in_bound,]

# Set default values for smoke indicator variables
in_cali_csn <- in_cali_csn %>%
  mutate(fire_dist=NA,closest_cl=NA, light=NA, med=NA, heavy=NA)

message("==========FINISHED READING==========")
message("Original dataset dimension: ", dim(in_cali_csn)[1], " observations and ", dim(in_cali_csn)[2], " features.")
...
for(i in 1:length(years)){
  ...
  for (d in as.list(dates)){
    ...
    if (dim(day_fire)[1] != 0){
      ...
      day_csn$fire_dist <- units::set_units(st_distance(day_csn$geometry, day_fire[closest_fire,]$geometry, by_element = TRUE), value=km)
...
write.csv(csn.merged,paste0(repo.dir, "data/merged/Merged_MISR_CSN_Matched_2000_2021.csv"), row.names = FALSE)
...
readLines(paste0(repo.dir, "errors-logs/errors_merge_misr_csn.Rout"))
